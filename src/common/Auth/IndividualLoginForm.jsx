...
import { loginIndividualUser, reactivateAccount, recoverIndividualAccount, } from "@/store/slices/individualAuth.slice";
...
const IndividualLoginForm = () => {
  ...
  const [reactivateIndividualAccount, setReactivateIndividualAccount] = useState(false);
  const { activeAccount, userErrorData } = useSelector((state) => state.individualAuth);
  ...
  const { mutate: loginMutation, isLoading } = useMutation({
    mutationFn: async (credentials) => {
      const response = await dispatch(loginIndividualUser(credentials)).unwrap();
      if (response?.success) {
        router.push("/individual/dashboard");
        setRecoverAccount(false);
        setReactivateIndividualAccount(false);
      }
    },
    onSuccess: () => {
      if (userErrorData?.isSoftDeleted) {
        ...
        setReactivateIndividualAccount(true);
      }
    },
    onError: (error) => {
  ...
  const { mutate: triggerReactivateIndividualAccount } = useMutation({
    mutationFn: async (credentials) => {
    ...
    onSuccess: (response) => {
      if (response?.success) {
        setReactivateIndividualAccount(false);
        router.push("/individual/dashboard");
      } else if (userErrorData?.isDeactivateAccount || activeAccount?.isDeactivatedAccount) {
        setReactivateIndividualAccount(true);
      } else {
        setReactivateIndividualAccount(false);
        router.push("/individual/dashboard");
  ...
  const { mutate: triggerRecoverIndividualAccount } = useMutation({
    mutationFn: async (credentials) => {
      return await dispatch(recoverIndividualAccount(credentials)).unwrap();
    },
    onSuccess: (response) => {
      if (response?.success) {
        setRecoverAccount(false);
  ...
  const handleConfirmReactivateAccount = async () => {
    await triggerReactivateIndividualAccount({ individualId: userErrorData?._id });
  };

  const handleConfirmRecoverAccount = async () => {
    await triggerRecoverIndividualAccount({ individualId: userErrorData?._id });
  };
    ...
    <>
      ...
      {reactivateIndividualAccount && (
        <div>
          ...
          >
            <RecoveryCardPopup
              setIsOpenSuccess={setReactivateIndividualAccount}
